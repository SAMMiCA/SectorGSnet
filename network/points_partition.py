...
__all__ = ["points_to_sector_fixed_ops", "points_to_sector_dynamic_ops"]


####################################
##    point to dynaimal sector    ##
####################################

@numba.jit(nopython=True)
def _points_to_sector_dynamic_ops_kernel(points,
                                        coor_to_sectoridx,
                                        sector_features,
                                        sector_coors,
    ...
    sector_coor = np.zeros(shape=(3,), dtype=np.int32) # record coordinate of sector
    ...
    for i in range(points.shape[0]):
        ...
        sector_coor[0] = rad_id  # id: 0~63
        sector_coor[1] = ang_id # id: 0~255
        sector_coor[2] = height_id
        ...
        if num < max_points_in_sector:
            sector_features[sectoridx, num,:4] = points[i]
            sector_features[sectoridx, num, 4] = rad_val
            sector_features[sectoridx, num, 5] = dis_val
            num_points_per_sector[sectoridx] += 1
    return sector_num

def points_to_sector_dynamic_ops(points,
                             sector_shape=[64, 256, 1],
                             max_points_in_sector=100,
                             max_sector=10000):
    ...
    num_points_per_sector = np.zeros(shape=(max_sector,), dtype=np.int32)  # 100000x1
    sector_features = np.zeros(
        shape=(max_sector, max_points_in_sector, points.shape[-1]+2), dtype=points.dtype)  # 10000*100*4+2(radius and distance)
    sector_coors = np.zeros(shape=(max_sector, 3), dtype=np.int32)  # 10000*3

    sector_num = _points_to_sector_dynamic_ops_kernel(
        points, coor_to_sectoridx,
        sector_features, sector_coors,num_points_per_sector,
        max_points_in_sector, max_sector)

    pts_in_sector = sector_features[:sector_num]  # p x n x 4+2 (x,y,z,i,rad,dis)
    coors_sector = sector_coors[:sector_num]  # p x 3
    num_points_per_sector = num_points_per_sector[:sector_num]  # p x 1
    return pts_in_sector, coors_sector, num_points_per_sector

##########################################
##   point to fixed sector - discard    ##
##########################################

@numba.jit(nopython=True)
def _points_to_sector_fixed_ops_kernel(points,
                                        coor_to_sectoridx,
                                        sector_features,
                                        sector_coors,
                                        num_points_per_sector,
                                        max_points_in_sector=100,
                                        max_sector=8000):
    # sector_size: 3.6degree 0.5m
    sector_num = 0
    sector_size = [0.625, 1.40625] # sector_shape=[64,256]
    sector_coor = np.zeros(shape=(3,), dtype=np.int32)

    for i in range(points.shape[0]): # All Points
        ang_id = np.floor(
            (math.atan2(points[i, 0], points[i, 1]) / math.pi * (180 / sector_size[1])) + 128)
        # start from 3rd phase. heading is x-asix 起始点从第三象限开始逆时针旋转(0->127)
        rad_val = math.sqrt(points[i, 0] ** 2 + points[i, 1] ** 2)  # 点到圆心距离
        # rad_val= np.linalg.norm([points[i,0],points[i,1]])
        rad_id = np.floor((rad_val-3.0) / sector_size[0]) # from 3.0 to 42.375 meters
        height_id = 0
        dis_val = math.sqrt(points[i, 0] ** 2 + points[i, 1] ** 2+ points[ i, 2] **2)
        ...
        if rad_id > 64 or rad_id < 0 :
            print("[Warning] rad_id out of range :")
            print(points[i,0], points[i,1])
            rad_id= 0

        sector_coor[1] = ang_id
        sector_coor[0] = rad_id
        sector_coor[2] = height_id
        sectoridx = coor_to_sectoridx[sector_coor[0], sector_coor[1], sector_coor[2]]  # 100x100x1
        if sectoridx == -1:
            sectoridx = sector_num
            if sector_num >= max_sector:
                break
...
def points_to_sector_fixed_ops(points,
                             sector_shape=[64, 256, 1],
                             max_points_in_sector=100,
                             max_sector=8000):

    # define cylinder_shape first : 128,64,1,
    sector_shape = tuple(np.round(sector_shape).astype(np.int32).tolist())
    coor_to_sectoridx = - np.ones(shape=sector_shape, dtype=np.int32)  # 64 x 256 x 1
    num_points_per_sector = np.zeros(shape=(max_sector,), dtype=np.int32)  # 80000
    sector_features = np.zeros(
        shape=(max_sector, max_points_in_sector, points.shape[-1]+2), dtype=points.dtype)  # 8000 * 100 * 4+2(radius and distance)
    sector_coors = np.zeros(shape=(max_sector, 3), dtype=np.int32)  # 8000*3

    sector_num = _points_to_sector_fixed_ops_kernel(
        points, coor_to_sectoridx,
        sector_features, sector_coors,num_points_per_sector,
        max_points_in_sector, max_sector)

    pts_in_sector = sector_features[:sector_num]  # p x n x 4+2 (x,y,z,i,rad,dis)
    sector_coors = sector_coors[:sector_num]  # p x 3
    num_points_per_sector = num_points_per_sector[:sector_num]  # p x 1
    return pts_in_sector, sector_coors, num_points_per_sector
